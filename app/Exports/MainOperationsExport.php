...
class MainOperationsExport implements FromQuery, WithChunkReading, WithHeadings, WithMapping
{
    protected $filters;
    ...
    public function query()
    {
        $query = MainOperation::query()
            ->with(['plant', 'machineType', 'machineNumber', 'task', 'employee', 'members', 'department'])
            ->where('status', 1);
    ...
    {
        return [
            '日付', '工場', '部門', '機台', '機台の番号', '作業', '小作業', '開始時間', '終了時間', '担当者', '担当メンバー', '合計時間',
        ];
    }
    ...
    public function map($mainOperation): array
    {
        return [
            ...
            optional($mainOperation->department)->name ?? '',
