 <header class="bg-lightgray-200 text-white">
        <nav class="p-6 mx-auto max-w-screen-lg flex items-center justify-between">
           <img src="/public/img/matsubun-logo.png " alt="">
            <NavLink routeName="home" componentName="Home">ホーム</NavLink>
            <div class="flex items-center space-x-6">
                
                <NavLink v-if="user" routeName="employees.index" componentName="MachineType/Index">従業員</NavLink>
                <NavLink v-if="user" routeName="machinetypes.index" componentName="MachineType/Index">機台</NavLink>
                <NavLink v-if="user" routeName="tasks.index" componentName="MachineType/Index">作業</NavLink>
                <!----------Auth----------->
                <div v-if="user" class="relative">
                    
                    <div @click="show = !show" 
                    class="flex items-center gap-2 px-3 py-1 rounded-lg 
                    hover:bg-slate-700 cursor-pointer":class="{'bg-slate-700':show}">
                        <p>{{ user.name }}</p>
                    ...
                    bg-slate-800 text-white rounded-lg border-slate-300
                ...
                <div v-else class="space-x-6">
                    <NavLink routeName="login" componentName="Auth/Login">ログイン</NavLink>
                    <NavLink routeName="register" componentName="Auth/Register">レジスター</NavLink>
                </div>
...
const deleteMO = (moId) => {
    Swal.fire({
        ...
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#e3342f', // 赤色ボタン
...
const refreshData = () => {
  location.reload(); // browser page ကို ပြန် reload လုပ်တယ်
};
