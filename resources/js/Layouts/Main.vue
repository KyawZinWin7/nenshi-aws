...
<template>
    <Head>
        <link rel="icon" type="image/png" href="/img/logo-nenshi.png" />
    </Head>
    ...
    <nav class="bg-white border-gray-200 dark:bg-gray-900">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 relative z-50">
            ...
            <!-- Menu Items -->
            <div 
                class="absolute md:static top-16 left-0 w-full md:w-auto 
                       transition-all duration-300 md:block"
                :class="mobileMenu ? 'block bg-white dark:bg-gray-900 z-50' : 'hidden'">

                <ul
                    class="font-medium flex flex-col md:flex-row md:space-x-8 
                    ...
                    <li v-if="user">
                        <Link 
                            ...
                            class="block py-2 px-3 rounded-sm md:p-0 text-sm md:text-base"
                            :class="{
                                'text-blue-700 bg-gray-100 md:bg-transparent md:text-blue-700 dark:text-blue-500': route().current('employees.index'),
                                'text-gray-900 hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent': !route().current('employees.index')
                            }" 
                            @click="closeMenu">
                            従業員
                        </Link>
                    ...
                    <li>
                        <Link 
                            :href="route('mainoperatons.completelist')" 
                            class="block py-2 px-3 rounded-sm md:p-0 text-sm md:text-base"
                            :class="{
