...
const props = defineProps({
  sizingoperations: { type: Object, required: true },
...
const exportExcel = async () => {
  ...
  try {
    const response = await axios.post('/sizingoperations/exportstore', data, {
      responseType: 'blob',
    });
    ...
    link.href = url;
    link.setAttribute('download', 'sizingoperation.xlsx');
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select v-model="form.machine_number"
                  class="mt-1 block w-full border rounded-md py-1.5 sm:py-2 px-2 sm:px-3 text-xs sm:text-sm">
                  <option value="">すべて</option>
                  <option v-for="num in 10" :key="num" :value="String(num)">{{ num }}</option>
                </select>
              </div>
            </div>
