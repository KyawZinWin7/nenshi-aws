...
//For Update machinenumber
const updateMachineNumber = () => {
  form.put(route("machinenumbers.update", machinenumber.id), {
    onSuccess: () => {
      Swal.fire({
        position: "top-end",
        icon: "success",
  ...
  <AdminLayout>
    ...
    <main class="mx-auto p-6 max-w-screen-sm min-h-screen">
      <div class="max-w-full px-6 py-6 bg-gray-100">
        ...
        <form class="w-full" @submit.prevent="updateMachineNumber">

          <div class="shadow sm:rounded-md sm:overflow-hidden bg-white">
            <div class="py-6 px-4 space-y-6 sm:p-6">
              <div>
                <h3 class="text-lg leading-6 font-medium text-gray-900">機械番号</h3>
              </div>
              ...
              <div>
                ...
                <p v-if="form.errors.plant_id" class="text-red-500 text-xs mt-1">{{ form.errors.plant_id }}</p>
              </div>
