...
let smalltask = usePage().props.smalltask.data;
const form = useForm({
    name: smalltask.name,
    machine_type_id: smalltask.machine_type_id.id,
})
...
//For Update Task
const updateTask = () => {
    form.put(route("smalltasks.update", smalltask.id), {
        onSuccess: () => {
            Swal.fire({
                position: "top-end",
                icon: "success",
                title: "小作業が正常に更新されました。",
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-6">
                                    ...
                                    <select v-model="form.machine_type_id"
                                        ...
                                        <option v-for="machinetype in machinetypes.data" :key="machinetype.id"
                                            :value="machinetype.id">
                                            {{ machinetype.name }}
                                        </option>
                                    </select>
                                    <div v-if="form.errors.machine_type_id" class="text-red-500 text-sm mt-1">{{
                                        form.errors.machine_type_id
                                        }}</div>
                                </div>
                            </div>
